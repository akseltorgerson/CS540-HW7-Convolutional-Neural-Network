...
class LeNet(nn.Module):
		def __init__(self, input_shape=(32, 32), num_classes=100):
				...
				# certain definitionsi

				self.conv1 = nn.Conv2d(in_channels=3, out_channels=6, kernel_size=5, stride=1, padding=0)
				self.relu1 = nn.ReLU()
				self.mxpl1 = nn.MaxPool2d(kernel_size=2, stride=2, padding=0)

				self.conv2 = nn.Conv2d(in_channels=6, out_channels=16, kernel_size=5, stride=1, padding=0)
				self.relu2 = nn.ReLU()
				self.mxpl2 = nn.MaxPool2d(kernel_size=2, stride=2, padding=0)
		...
		def forward(self, x):
				...
				# fully connect, with ReLU
				x = nn.functional.relu(self.fc1(x))

				# add to dict
				shape_dict[4] = list(x.shape)

				# fully connect again, with ReLU
				x = nn.functional.relu(self.fc2(x))

				# add to dict
				shape_dict[5] = list(x.shape)

				# fully connect once more, no ReLU
				x = self.fc3(x)
